...
namespace MedReminder.Pages
{
    public partial class HomePage : AuthPage
    {
        ...
        public HomePage()
        {
            ...
            _medicationService = services.GetRequiredService<MedicationService>();
            _residentService = services.GetRequiredService<ResidentService>();

            BindingContext = _vm;

            // If you keep hardcoded items in XAML, this won’t break anything.
            // If you want "00..23", you can keep this (it will replace XAML items).
            if (HourPicker != null)
            {
        ...
        private async void OnSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            await Shell.Current.GoToAsync($"viewResident?id={m.ResidentId.Value}");
        }

        // IMPORTANT: HomePage.xaml uses CheckBox.CheckedChanged
        private async void OnDoneCheckedChanged(object sender, CheckedChangedEventArgs e)
        {
            ...
            await _medicationService.SaveAsync(all);
        }
        ...
        private void RunSearch()
        {
            ...
            {
                var resident = _allResidents.FirstOrDefault(r => r.Id == med.ResidentId);
                med.ResidentName = resident?.Name;

                if (hasResidentFilter &&
                    (resident == null ||
                     !resident.Name.Contains(residentQuery, StringComparison.OrdinalIgnoreCase)))
                    continue;
